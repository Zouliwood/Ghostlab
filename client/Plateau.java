...
public class Plateau {
   ...
   void addWall(int direction, int nbrCase, int newX, int newY){
        ...
        if (player.isBlocked(direction, nbrCase)){
            if (direction==0){//haut
                if (player.y+1<plateau.length)this.plateau[player.x][player.y+1]=2;
            } else if (direction==1) {//droite
                if (player.x+1<plateau[0].length)this.plateau[player.x+1][player.y]=2;
            } else if (direction==2) {//bas
                if (player.y-1>=0)this.plateau[player.x][player.y+1]=2;
            }else{//gauche
                if (player.x-1>=0)this.plateau[player.x-1][player.y]=2;
            }
        }
   }
